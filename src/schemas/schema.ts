// schema.ts
import { pgTable, uuid, varchar, boolean, timestamp, integer, real, jsonb, text, pgEnum, index, uniqueIndex } from 'drizzle-orm/pg-core';
import { relations } from 'drizzle-orm';

// Enums
export const userTierEnum = pgEnum('user_tier', ['free', 'premium', 'enterprise']);
export const subscriptionStatusEnum = pgEnum('subscription_status', ['active', 'canceled', 'past_due', 'trialing']);
export const detectionStatusEnum = pgEnum('detection_status', ['processing', 'completed', 'failed']);
export const confidenceLevelEnum = pgEnum('confidence_level', ['low', 'medium', 'high', 'very_high']);
export const providerEnum = pgEnum('provider', ['email', 'google', 'github', 'microsoft']);
export const actionEnum = pgEnum('action', ['detection', 'api_call', 'export', 'share']);
export const reportTypeEnum = pgEnum('report_type', ['false_positive', 'false_negative', 'bug', 'other']);
...
export const detectionResults = pgTable('detection_results', {
    ...
    [index('detection_results_user_created_idx').on(table.userId, table.createdAt),
    index('detection_results_status_idx').on(table.status),
    index('detection_results_bookmarked_idx').on(table.userId, table.isBookmarked),]
));

// Frame Analysis Table
export const frameAnalyses = pgTable('frame_analyses', {
    id: uuid('id').primaryKey().defaultRandom(),
    detectionResultId: uuid('detection_result_id').notNull().references(() => detectionResults.id, { onDelete: 'cascade' }),
...
export const reports = pgTable('reports', {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
    detectionResultId: uuid('detection_result_id').references(() => detectionResults.id, { onDelete: 'set null' }),
...
export const paymentProviderEnum = pgEnum('payment_provider', ['stripe', 'paypal', 'paddle']);
...
export const paymentStatusEnum = pgEnum('payment_status', ['succeeded', 'pending', 'failed', 'refunded', 'canceled']);
...
export const subscriptions = pgTable('subscriptions', {
    ...
    uniqueIndex('subscriptions_provider_sub_id_idx').on(table.providerSubscriptionId),
...
export const paymentMethods = pgTable('payment_methods', {
    ...
    uniqueIndex('payment_methods_provider_payment_method_id_idx').on(table.providerPaymentMethodId),]
));
...
export const detectionResultsRelations = relations(detectionResults, ({ one, many }) => ({
    user: one(users, {
